...
func parentExcludeJoin(dbName string) (joinClause, whereCondition string) {
	joinClause = `LEFT JOIN (
		...
		INNER JOIN wisps parent ON parent.id = COALESCE(wd.depends_on_issue_id, wd.depends_on_wisp_id, wd.depends_on_external)
...
func Scan(db *sql.DB, dbName string, maxAge, purgeAge, mailDeleteAge, staleIssueAge time.Duration) (*ScanResult, error) {
	...
	staleQuery := `
		...
		AND i.id NOT IN (
			...
			INNER JOIN issues dep ON COALESCE(d.depends_on_issue_id, d.depends_on_wisp_id, d.depends_on_external) = dep.id
			WHERE dep.status IN ('open', 'in_progress')
		)
		AND i.id NOT IN (
			SELECT DISTINCT COALESCE(d.depends_on_issue_id, d.depends_on_wisp_id, d.depends_on_external) FROM dependencies d
	...
	danglingQuery := `
		...
		LEFT JOIN wisps parent ON parent.id = COALESCE(wd.depends_on_issue_id, wd.depends_on_wisp_id, wd.depends_on_external)
...
func AutoClose(db *sql.DB, dbName string, staleAge time.Duration, dryRun bool) (*AutoCloseResult, error) {
	...
	whereClause := fmt.Sprintf(`
		...
		AND i.id NOT IN (
			...
			INNER JOIN `+"`%s`"+`.issues dep ON COALESCE(d.depends_on_issue_id, d.depends_on_wisp_id, d.depends_on_external) = dep.id
			WHERE dep.status IN ('open', 'in_progress')
		)
		AND i.id NOT IN (
			SELECT DISTINCT COALESCE(d.depends_on_issue_id, d.depends_on_wisp_id, d.depends_on_external) FROM `+"`%s`"+`.dependencies d
...
func batchDeleteRows(ctx context.Context, db *sql.DB, idQuery string, cutoffArg time.Time, primaryTable string, auxTables []string) (int, error) {
	...
	for {
		...
		delReverse := fmt.Sprintf("DELETE FROM wisp_dependencies WHERE COALESCE(depends_on_issue_id, depends_on_wisp_id, depends_on_external) IN %s", inClause)
